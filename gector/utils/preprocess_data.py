...
def perfect_align(t, T, insertions_allowed=0,
    ...
    for i in range(len(t) + 1):  # Go inclusive
        for j in range(len(T) + 1):  # Go inclusive
            for q in range(insertions_allowed + 1):  # Go inclusive
                ...
                if q < insertions_allowed:  # 直接插入j - k-1 个字符来对齐 但是gector 不允许插入 下面的用不上
    ...
    # Solution is in the dp[len(t), len(T), *]. Backtracking from there. 回溯
    alignment = []
    i = len(t)
    j = len(T)
...
'''
读corr文件和incorr文件
然后将处理后的内容（tag）写到out文件
写入格式： 源token+分隔符+label
'''
...
def convert_tagged_line(line, delimeters=SEQ_DELIMETERS):
    ...
    source_tokens = [x.split(label_del)[0]
                     for x in line.split(delimeters['tokens'])][1:]  # 除去start
