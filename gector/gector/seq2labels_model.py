...
class Seq2Labels(Model):
    ...
    def forward(self,  # type: ignore
        ...
        # 生成mask 不把attention放在padding部分 padding部分为0 其他为1
        ...
        # 把经过time distributed 处理的labels 和detect tag送入softmax层，并转回三维
        ...
        if labels is not None and d_tags is not None:
            # 标签平滑 防止过拟合，例 一个二分类问题 label_smoothing=0.2 那么得到的概率矩阵为 [0,1]*(1-0.2)+0.2/2 = [0.1,0.9]
            # 公式为((1-epsilon) * inputs) + (epsilon / K) 其中 epsilon对应上面的0.2
